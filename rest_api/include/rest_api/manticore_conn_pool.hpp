#ifndef MANTICORE_CONN_POOL_HPP
#define MANTICORE_CONN_POOL_HPP
...
typedef struct manticore_connection_s manticore_connection_t;
#endif

struct manticore_pool_config_t
{
    std::string db;
    std::string server;
    std::string user;
...
class MYSQLPP_EXPORT ManticoreConnectionPool
{
public:
    // using Connection = redis_client::ManticoreClient;
    using ConnectionPtr = std::unique_ptr<Connection>;

    ManticoreConnectionPool(){}
    virtual ~ManticoreConnectionPool()
    {
        assert(empty());
    }
...
class MYSQLPP_EXPORT ManticoreScopedConnection
{
public:
  explicit ManticoreScopedConnection(ManticoreConnectionPool& pool)
      : pool_(&pool), conn_(pool.grab())
  {}

  ~ManticoreScopedConnection()
  {
      if (pool_ && conn_) {
          pool_->release(conn_);
      }
  ...
  ManticoreScopedConnection(const ManticoreScopedConnection&) = delete;
  ManticoreScopedConnection& operator=(const ManticoreScopedConnection&) = delete;

  ManticoreScopedConnection(ManticoreScopedConnection&& other) noexcept
  ...
  ManticoreScopedConnection& operator=(ManticoreScopedConnection&& other) noexcept
  {
      if (this != &other) {
          if (pool_ && conn_) {
              pool_->release(conn_);
...
class SimpleConnectionPool : public mysqlpp::ManticoreConnectionPool
{
...
public:
    ...
    {
        ++conns_in_use_;
        return mysqlpp::ManticoreConnectionPool::grab();
    }
    mysqlpp::Connection* safe_grab() override
    {
        ++conns_in_use_;
        return mysqlpp::ManticoreConnectionPool::safe_grab();
    }
    void release(const mysqlpp::Connection* pc) override
    {
        mysqlpp::ManticoreConnectionPool::release(pc);
        --conns_in_use_;
    }
protected:
